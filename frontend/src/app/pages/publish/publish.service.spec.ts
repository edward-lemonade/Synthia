...
import { ProjectMetadata, ProjectFront, WaveformData } from '@shared/types';
import { of } from 'rxjs';
...
describe('PublishService', () => {
	...
	it('should load export successfully', async () => {
		...
		const mockWaveformData: WaveformData = {
			...
			sampleRate: 44100,
			channels: 2,
			peaks: new Float32Array([0.5, 0.3, 0.7])
		};
		const mockWaveform = { waveformData: mockWaveformData };
		
		(ApiService as any)._instance.routes.getMyProjectExport.and.returnValue(
			Promise.resolve({ data: mockExport })
		);
		(ApiService as any)._instance.routes.getMyProjectWaveform.and.returnValue(
			Promise.resolve({ data: mockWaveform })
		);
		...
		expect(service.cachedAudioFile?.waveformData).toEqual(mockWaveformData);
	});
