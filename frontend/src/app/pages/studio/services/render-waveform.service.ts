	...
	private drawWaveformViewport(
		...
		if (peaksLength <= maxBars) {
			...
			for (let i = 0; i < peaksLength; i++) {
				...
				const x = startPx + (i * actualBarStep);
			...
			for (let i = 0; i < maxBars; i++) {
				const x = startPx + (i * barStep); // Use original barStep when downsampling
